...
7. Preserve identity: long black hair, blunt bangs, red headband, yellow warning-sign clip on her right side (viewer left), blue left-arrow clip on her left side (viewer right), violet eyes, black outfit with dark-red accents, a black pleated over-knee midi skirt whose hem is clearly below both kneecaps in the upper-calf area, black knee socks, and black shoes. The skirt must never drift to knee-length, above-knee, or miniskirt proportions, including in walking, turning, or lifted-leg poses.
...
9. Choose exactly one neck accessory independently for every job: either `red_scarf` or `traffic_light_pendant`. The one-accessory rule is exclusively an either/or rule between those two neck accessories; it does not apply to fixed costume details. Komichi's canonical waist ornament/tassel cluster is part of her base outfit, does not occupy the neck-accessory slot, and must remain whenever the waist is visible. The neck-accessory choice must be random with a long-run 50/50 target and must not depend on shot scale, crop, pose, placement side, or face size. A close-up may use the scarf; a full-body or environmental shot may use the pendant. Never show scarf and pendant together. When using the pendant, use the black sailor-style neckline with dark-red stripes from the face references and no scarf. In a genuine side-facing or back-facing pose, the selected pendant may be partly or fully hidden naturally by the neck, collar, hair, or body geometry; do not reject the image solely because the pendant is not visible when that occlusion is physically plausible and no scarf or substitute neck accessory appears. Record the chosen value in job metadata and `generation-state.json`; avoid the same choice more than twice in a row when possible.
...
- the black pleated skirt hem is at the kneecaps or above, reads as a knee-length/above-knee/miniskirt, or otherwise fails to read as an over-knee midi skirt reaching the upper calf;
- the two viewer-right tear moles are missing, merged, misplaced, or not vertically stacked when the face is visible;
- the selected neck accessory is missing without a physically plausible pose-based occlusion, scarf and pendant both appear, a substitute neck accessory appears, or the neck-accessory choice tracks the shot scale instead of the per-job random choice; a naturally hidden pendant in a genuine side/back view is allowed when no scarf or substitute neck accessory is present; the canonical waist ornament is a separate fixed costume detail and must not be removed or counted as a second neck accessory;
